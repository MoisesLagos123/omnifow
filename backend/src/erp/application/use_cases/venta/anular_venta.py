...
class AnularVentaUseCase:
    ...
    def execute(self, cmd: AnularVentaCommand) -> AnularVentaResult:
        # Variable de Clock por compatibilidad con la firma original;
        # actualmente no se usa, pero la mantenemos por si auditorías
        # externas quieren stampear "ahora_anulacion" antes de delegar.
        _ = self._clock.now()

        # Abrimos el UoW del padre para las lecturas previas + delegado.
        # El delegado ProcesarDevolucionUseCase también hace `with self._uow:`
        # internamente — el UoW soporta re-entry (contador) y comparte la
        # misma sesión entre ambos niveles, por lo que esto es seguro y
        # mantiene la atomicidad del flujo completo (lectura + delegado +
        # audit + recarga viven en la misma sesión).
        with self._uow:
            # Verificar venta antes de delegar
            ...
            # Contexto con permiso devolucion.crear para que el delegado no
            # rechace (el caller ya tiene venta.anular que es el permiso de
            # entrada aquí). Creamos un contexto extendido.
            ...
            # Audit adicional venta.anular para compatibilidad con queries
            # existentes del audit log.
            ...
            # Recargar la venta para obtener el estado actualizado tras la
            # devolución (puede haber pasado a ANULADA).
            ...
            return AnularVentaResult(
                ...
                movimientos_inventario_ids=(),  # compat: vacío (info en devolucion)
